...
public class CollectionsPathRetreiver
{
	...
	public List getNestedProperty(Map target, String propertyPath)
	{
		...
		//Below lines of code gets the first path segment and identifies if more path segments exists
		...
		if(indexOfSqrBracketOpen > -1)
		{
			...
			//current property could be multi level indexed property similar to multi dimentional array like [1][3][2]
			//below line recurses and gets the values. Also handles wild card indexes [*][1][3]
			List vals = getMultilevelIndexedProperty(property, escapedPropertyPathSegment.substring(escapedPropertyPathSegmentBase.length()));
			if(vals != null)
			{
				if(escapedRemainingPropertyPath != null)
				{
					...
					for(int i = 0 ; i < vals.size() ; i++)
					{
						...
						{
							System.out.println("List element is not a Map but has remaining path to process:"+unescapePropertyPath(escapedRemainingPropertyPath));
						}
					}
				}
				else
			...
			if(escapedRemainingPropertyPath != null)
			{
				if(escapedPropertyPathSegmentBase.equalsIgnoreCase("*"))
				{
					...
					for (Entry entry : entrySet)
					{
						...
						{
							List nestedVal = getNestedMappedProperty((Map) val, escapedRemainingPropertyPath, escapedPropertyPathSegmentBase);
							if(nestedVal != null)
				...
				{
					List val = getNestedMappedProperty(target, escapedRemainingPropertyPath, escapedPropertyPathSegmentBase);
					if(val != null)
						ret.addAll(val);
					else
						return null;
			...
			String escapedPropertyPathSegmentBase)
	{
			...
			if(escapedRemainingPropertyPath != null)
			{
				...
				for (Iterator iterator = keySet.iterator(); iterator.hasNext();)
				{
					...
					if(key != null)
					{
						...
						{
							System.out.println("key is not Map but has remaining path without index to process:"+unescapePropertyPath(escapedRemainingPropertyPath));
						}
					}
				}
			}
		...
		//more property path segments are there to process, so it must be a Map. Proceed to next property path segment
...
//			throw new RuntimeException("Map cannot have index as key");
		...
		if(indexStr.matches("[0-9]+"))
		{
			...
			{
				logger.info("Property index out of range returning nothing. Target size:{} index:{}",target.size(), index);
		...
		else if(indexStr.matches("[*]"))
		{
			...
			{
				//There is no sub path and the current index is wildcard, return the incoming list
				return target;
			}
		}
